...
// defining operator to access (i,j) element of the matrix
double BlockMatrix::operator()(std::size_t i, std::size_t j) const{
    ...
    else
    {
        for (const Block & block : blocks)
        {
            ...
            {
                // access proper indexes element using the operator () implemented in block class
                elem = block(
...
double& BlockMatrix::operator() (std::size_t i, std::size_t j){
    for (Block & block : blocks)
    {
        ...
        if (indexes_in_block (i, j, block))
        {
            // change proper indexes element using the operator () implemented in block class and return
            return block(i - block.get_top_left_row(), j - block.get_top_left_col());
        }
    }
    // If (i,j) are not found in the matrix a 1x1 0 block is added
    // Create a single element block (by constructor)
